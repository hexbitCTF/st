...
xfinishdraw(void)
{
	...
	if (win.cw == im->cw && win.ch == im->ch) {
		XImage *xi = XCreateImage(xw.dpy, xw.vis,
			xw.depth, ZPixmap, 0,
			(char *)im->pixels, im->width, im->height, 32,
			im->width * 4);
				if (xi) {
					xi->byte_order = sixelbyteorder;
					xi->bitmap_bit_order = MSBFirst;
					XPutImage(xw.dpy, (Drawable)im->pixmap, dc.gc, xi,
					...
					use_imlib = im->transparent && !im->clipmask;
				}
			}
			if (use_imlib) {
				...
				imlib_context_set_anti_alias(im->transparent ? 0 : 1);
